...
class HotmailSignup:
    ...
    def move_window_to_background(self):
        """Move Chrome driver window to background (Windows only)"""
        time.sleep(1)  # รอให้ Chrome driver โหลด
        try:
            def enum_handler(hwnd, lParam):
                title = win32gui.GetWindowText(hwnd)
                if "Chrome" in title:
                    # ใส่ SWP_NOACTIVATE เพื่อไม่ดึง focus
                    win32gui.SetWindowPos(hwnd, win32con.HWND_BOTTOM, 0, 0, 0, 0,
                                        win32con.SWP_NOMOVE | win32con.SWP_NOSIZE | win32con.SWP_NOACTIVATE)
                    print(f"Moved window '{title}' to background.")
            win32gui.EnumWindows(enum_handler, None)
        except Exception as e:
            print(f"Failed to move Chrome to background: {e}")
    ...
    def setup_driver(self):
        ...
        try:
            ...
            options.add_argument("--window-size=380,400")
            options.add_argument("--window-position=-32000,-32000")  

            self.driver = uc.Chrome(options=options)
            self.driver.set_window_size(380, 400)  
            self.wait = WebDriverWait(self.driver, 20)
            self.move_window_to_background()
    ...
    def signup_with_retry(self, email, password, first_name, last_name, account_index):
        """Main function with retry logic and file management on success"""
        for attempt in range(self.max_retries):
            try:
                ...
                # บันทึกข้อมูลลง mail.txt
                save_success_account(email, password)

                # ลบข้อมูลใน infomail.txt ที่ใช้แล้ว
                remove_used_account([account_index * 4, account_index * 4 + 1, account_index * 4 + 2, account_index * 4 + 3])
...
def save_success_account(email, password):
    """บันทึกอีเมลและรหัสผ่านลงใน mail.txt"""
    with open("mail.txt", "a", encoding="utf-8") as f:
        f.write(f"{email}\n{password}\n\n")


def remove_used_account(line_numbers_to_remove):
    """
    ลบบรรทัดที่ใช้แล้วใน infomail.txt
    line_numbers_to_remove: list of line numbers to remove (0-based)
    """
    try:
        with open("infomail.txt", "r", encoding="utf-8") as f:
            lines = f.readlines()

        # กรองบรรทัดที่ไม่ได้อยู่ใน line_numbers_to_remove
        new_lines = [line for idx, line in enumerate(lines) if idx not in line_numbers_to_remove]

        with open("infomail.txt", "w", encoding="utf-8") as f:
            f.writelines(new_lines)

        return True
    except Exception as e:
        print(f"[ERROR] Failed to update infomail.txt: {str(e)}")
        return False


def signup_hotmail(email, password, first_name, last_name, max_retries=3, debug=True, account_index=0):
    """
    ...
    Args:
        ...
        account_index: index of the account in infomail.txt
    """
    signup_bot = HotmailSignup(max_retries=max_retries, debug=debug)
    try:
        result = signup_bot.signup_with_retry(email, password, first_name, last_name, account_index=account_index)
        return result
    finally:
        signup_bot.cleanup()
...
if __name__ == "__main__":
    ...
    try:
        ...
        for index, block in enumerate(content):
            ...
            if len(lines) != 4:
                print(f"[WARNING] Invalid data at block {index + 1}")
                continue
            ...
            if success:
                ...
                print(f"Failed to sign up: {email}")
